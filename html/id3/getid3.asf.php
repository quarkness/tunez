...
//								return false;
			...
			case ASF_Content_Description_Object:
				...
				foreach ($ASFcommentKeysToCopy as $keytocopyfrom => $keytocopyto) {
					if (!empty($ThisFileInfo['asf']['content_description'][$keytocopyfrom.'_ascii'])) {
						$ThisFileInfo['asf']['comments']["$keytocopyto"][] = trim($ThisFileInfo['asf']['content_description'][$keytocopyfrom.'_ascii']);
					}
				}
				break;
			...
			case ASF_Extended_Content_Description_Object:
				...
				for ($ExtendedContentDescriptorsCounter = 0; $ExtendedContentDescriptorsCounter < $ThisFileInfo['asf']['extended_content_description']['content_descriptors_count']; $ExtendedContentDescriptorsCounter++) {
					...
					switch (strtolower($ThisFileInfo['asf']['extended_content_description']['content_descriptors'][$ExtendedContentDescriptorsCounter]['name_ascii'])) {
						case 'wm/albumartist':
						case 'artist':
						...
						case 'wm/genre':
						case 'genre':
							$OriginalASFgenre = $ThisFileInfo['asf']['extended_content_description']['content_descriptors'][$ExtendedContentDescriptorsCounter]['value_ascii'];
							require_once(GETID3_INCLUDEPATH.'getid3.id3v1.php');
							$CleanedGenre = LookupGenre(LookupGenre($OriginalASFgenre, true)); // convert to standard GenreID and back to standard spelling/capitalization
							if ($CleanedGenre && ($CleanedGenre != $OriginalASFgenre)) {
								$OriginalASFgenre = $CleanedGenre;
							}
							$ThisFileInfo['asf']['comments']['genre'][] = $OriginalASFgenre;
							break;
	...
	if (isset($ThisFileInfo['audio'])) {
		$ThisFileInfo['audio']['lossless']           = (isset($ThisFileInfo['audio']['lossless'])           ? $ThisFileInfo['audio']['lossless']           : false);
		$ThisFileInfo['audio']['dataformat']         = (!empty($ThisFileInfo['audio']['dataformat'])        ? $ThisFileInfo['audio']['dataformat']         : 'asf');
	}
	if (!empty($ThisFileInfo['video']['dataformat'])) {
		$ThisFileInfo['video']['lossless']           = (isset($ThisFileInfo['audio']['lossless'])           ? $ThisFileInfo['audio']['lossless']           : false);
		$ThisFileInfo['video']['pixel_aspect_ratio'] = (isset($ThisFileInfo['audio']['pixel_aspect_ratio']) ? $ThisFileInfo['audio']['pixel_aspect_ratio'] : (float) 1);
		$ThisFileInfo['video']['dataformat']         = (!empty($ThisFileInfo['video']['dataformat'])        ? $ThisFileInfo['video']['dataformat']         : 'asf');
	}
