...
function getQuicktimeHeaderFilepointer(&$fd, &$ThisFileInfo, $ReturnAtomData=true, $ParseAllPossibleAtoms=false) {
	...
	while ($offset < $ThisFileInfo['avdataend']) {
		...
		switch ($atomname) {
			...
			default:
				$atomHierarchy = array();
				$ThisFileInfo['quicktime']["$atomname"] = QuicktimeParseAtom($atomname, $atomsize, fread($fd, $atomsize), $ThisFileInfo, $offset, $atomHierarchy, $ParseAllPossibleAtoms);
				break;
		}
...
function QuicktimeParseAtom($atomname, $atomsize, $atomdata, &$ThisFileInfo, $baseoffset, &$atomHierarchy, $ParseAllPossibleAtoms) {
	...
	switch ($atomname) {
		...
		case 'rmda': // Reference Movie Descriptor Atom
			$atomstructure['subatoms'] = QuicktimeParseContainerAtom($atomdata, $ThisFileInfo, $baseoffset + 8, $atomHierarchy, $ParseAllPossibleAtoms);
			break;
		...
		case '©enc':
			...
			CopyToAppropriateCommentsSection($atomname, $atomstructure['data'], $ThisFileInfo);
			break;
		...
		case 'cmvd': // Compressed MooV Data atom
			...
			$atomstructure['unCompressedSize'] = BigEndian2Int(substr($atomdata,  0, 4));

			$CompressedFileData = substr($atomdata, 4);
			if ($UncompressedHeader = @gzuncompress($CompressedFileData)) {
$fpoo = fopen('foo.dat', 'wb');
$better = str_replace('nsav'."\x00\x00\x00\x01", 'nsav'."\x00\x00\x00\x00", $UncompressedHeader);
fwrite($fpoo, BigEndian2String(strlen($better), 4).gzcompress($better));
fclose($fpoo);
				$atomstructure['subatoms'] = QuicktimeParseContainerAtom($UncompressedHeader, $ThisFileInfo, 0, $atomHierarchy, $ParseAllPossibleAtoms);
			} else {
				$ThisFileInfo['warning'] .= "\n".'Error decompressing compressed MOV atom at offset '.$atomstructure['offset'];
			}
			break;
			...
			for ($i = 0; $i < $atomstructure['number_entries']; $i++) {
				...
				if ($atomstructure['sample_description_table'][$i]['encoder_vendor'] == chr(0).chr(0).chr(0).chr(0)) {
					...
					switch ($atomstructure['sample_description_table'][$i]['data_format']) {
						case 'mp4v':
							$ThisFileInfo['warning'] .= "\n".'This version ('.GETID3VERSION.') of getID3() does not fully support MPEG-4 audio/video streams';
							break;
		...
		case 'meta': // METAdata atom
			...
			while ($NextTagPosition < strlen($atomdata)) {
				$metaItemSize = BigEndian2Int(substr($atomdata, $NextTagPosition - 4, 4)) - 4;
				...
				CopyToAppropriateCommentsSection($metaItemKey, $metaItemData, $ThisFileInfo);
			}
			break;

		case 'ftyp': // FileTYPe (?) atom (for MP4 it seems)
			$atomstructure['signature'] =               substr($atomdata,  0, 4);
			$atomstructure['unknown_1'] = BigEndian2Int(substr($atomdata,  4, 4));
		...
		case 'nsav': // NoSAVe atom
			...
			$atomstructure['data'] = BigEndian2Int(substr($atomdata,  0, 4));
			break;
...
function QuicktimeParseContainerAtom($atomdata, &$ThisFileInfo, $baseoffset, &$atomHierarchy, $ParseAllPossibleAtoms) {
	...
	while ($subatomoffset < strlen($atomdata)) {
		...
		$atomstructure[$subatomcounter] = QuicktimeParseAtom($subatomname, $subatomsize, $subatomdata, $ThisFileInfo, $baseoffset + $subatomoffset, $atomHierarchy, $ParseAllPossibleAtoms);
