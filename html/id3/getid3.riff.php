...
function getRIFFHeaderFilepointer(&$fd, &$ThisFileInfo) {
	...
	switch ($arraykeys[0]) {
		...
		case 'RMP3':  // RMP3 is identical to WAVE, just renamed. Used by [unknown program] when creating RIFF-MP3s
			...
			if (isset($ThisFileInfo['RIFF']['raw']['fmt ']['wFormatTag'])) {
				switch ($ThisFileInfo['RIFF']['raw']['fmt ']['wFormatTag']) {
					case 0x08AE: // ClearJump LiteWave
						$ThisFileInfo['audio']['bitrate_mode'] = 'vbr';
						$ThisFileInfo['audio']['dataformat']   = 'litewave';
						...
						$ThisFileInfo['RIFF']['litewave']['raw']['compression_method'] = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 18, 1));
						$ThisFileInfo['RIFF']['litewave']['raw']['compression_flags']  = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 19, 1));
						$ThisFileInfo['RIFF']['litewave']['raw']['m_dwScale']          = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 20, 4));
						$ThisFileInfo['RIFF']['litewave']['raw']['m_dwBlockSize']      = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 24, 4));
						$ThisFileInfo['RIFF']['litewave']['raw']['m_wQuality']         = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 28, 2));
						$ThisFileInfo['RIFF']['litewave']['raw']['m_wMarkDistance']    = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 30, 2));
						$ThisFileInfo['RIFF']['litewave']['raw']['m_wReserved']        = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 32, 2));
						$ThisFileInfo['RIFF']['litewave']['raw']['m_dwOrgSize']        = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 34, 4));
						$ThisFileInfo['RIFF']['litewave']['raw']['m_bFactExists']      = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 38, 2));
						$ThisFileInfo['RIFF']['litewave']['raw']['m_dwRiffChunkSize']  = LittleEndian2Int(substr($ThisFileInfo['RIFF']['WAVE']['fmt '][0]['data'], 40, 4));

						//$ThisFileInfo['RIFF']['litewave']['quality_factor'] = (int) round((2000 - $ThisFileInfo['RIFF']['litewave']['raw']['m_dwScale']) / 20);
						$ThisFileInfo['RIFF']['litewave']['quality_factor'] = $ThisFileInfo['RIFF']['litewave']['raw']['m_wQuality'];

						$ThisFileInfo['RIFF']['litewave']['flags']['raw_source']    = InverseBoolean($ThisFileInfo['RIFF']['litewave']['raw']['compression_flags'] & 0x01);
						$ThisFileInfo['RIFF']['litewave']['flags']['vbr_blocksize'] = InverseBoolean($ThisFileInfo['RIFF']['litewave']['raw']['compression_flags'] & 0x02);
						$ThisFileInfo['RIFF']['litewave']['flags']['seekpoints']    =        (bool) ($ThisFileInfo['RIFF']['litewave']['raw']['compression_flags'] & 0x04);

						if ($ThisFileInfo['RIFF']['litewave']['raw']['m_wQuality'] == 100) {
							$ThisFileInfo['audio']['lossless'] = true;
						} else {
							$ThisFileInfo['audio']['lossless'] = false;
						}
				...
				switch (@$ThisFileInfo['audio']['dataformat']) {
					...
					case 'litewave':
						if (($ThisFileInfo['avdataend'] - $ThisFileInfo['filesize']) == 1) {
							...
							// Short by more than one byte, throw error
							$ThisFileInfo['warning'] .= "\n".'Probably truncated file - expecting '.$ThisFileInfo['RIFF'][$arraykeys[0]]['data'][0]['size'].' bytes of data, only found '.($ThisFileInfo['filesize'] - $ThisFileInfo['avdataoffset']).' (short by '.($ThisFileInfo['RIFF'][$arraykeys[0]]['data'][0]['size'] - ($ThisFileInfo['filesize'] - $ThisFileInfo['avdataoffset'])).' bytes)';
		...
		case 'AVI ':
			...
			if (isset($ThisFileInfo['RIFF'][$arraykeys[0]]['movi']['offset'])) {
				...
				if ($ThisFileInfo['avdataend'] > $ThisFileInfo['filesize']) {
					$ThisFileInfo['warning'] .= "\n".'Probably truncated file - expecting '.$ThisFileInfo['RIFF'][$arraykeys[0]]['movi']['size'].' bytes of data, only found '.($ThisFileInfo['filesize'] - $ThisFileInfo['avdataoffset']).' (short by '.($ThisFileInfo['RIFF'][$arraykeys[0]]['movi']['size'] - ($ThisFileInfo['filesize'] - $ThisFileInfo['avdataoffset'])).' bytes)';
					$ThisFileInfo['avdataend'] = $ThisFileInfo['filesize'];
				}
			}
		...
		case 'AIFC':
			...
			if (isset($ThisFileInfo['RIFF'][$arraykeys[0]]['COMM'][0]['data'])) {
				...
				if ($ThisFileInfo['RIFF'][$arraykeys[0]]['COMM'][0]['size'] > 18) {
					...
					switch ($ThisFileInfo['RIFF']['audio']['codec_name']) {
						case 'NONE':
						...
						case '':
							switch ($ThisFileInfo['RIFF']['audio']['codec_fourcc']) {
								// http://developer.apple.com/qa/snd/snd07.html
								case 'sowt':
									$ThisFileInfo['RIFF']['audio']['codec_name'] = 'Two\'s Compliment Little-Endian PCM';
									$ThisFileInfo['audio']['lossless'] = true;
									break;

								case 'twos':
									$ThisFileInfo['RIFF']['audio']['codec_name'] = 'Two\'s Compliment Big-Endian PCM';
									$ThisFileInfo['audio']['lossless'] = true;
									break;
	...
	if (!empty($ThisFileInfo['RIFF']['raw']['fmt ']['wBitsPerSample']) && ($ThisFileInfo['RIFF']['raw']['fmt ']['wBitsPerSample'] > 0)) {
		$ThisFileInfo['audio']['bits_per_sample'] = $ThisFileInfo['RIFF']['raw']['fmt ']['wBitsPerSample'];
	}
...
function RIFFparseWAVEFORMATex($WaveFormatExData) {
	...
	$WaveFormatEx['raw']['wBitsPerSample']  = LittleEndian2Int(substr($WaveFormatExData, 14, 2));
	$WaveFormatEx['raw']['cbSize']          = LittleEndian2Int(substr($WaveFormatExData, 16, 2));
	...
	$WaveFormatEx['bits_per_sample'] = $WaveFormatEx['raw']['wBitsPerSample'];
...
function RIFFwFormatTagLookup($wFormatTag) {
	...
	if (empty($RIFFwFormatTagLookup)) {
		...
		$RIFFwFormatTagLookup[0x0050] = 'Microsoft MPEG Layer-1 or Layer-2';
		...
		$RIFFwFormatTagLookup[0x00FF] = 'AAC';
		...
		$RIFFwFormatTagLookup[0x08AE] = 'ClearJump LiteWave';
		...
		$RIFFwFormatTagLookup[0x2001] = 'DTS';
		$RIFFwFormatTagLookup[0x2002] = 'WAVE_FORMAT_14_4';
		$RIFFwFormatTagLookup[0x2003] = 'WAVE_FORMAT_28_8';
		$RIFFwFormatTagLookup[0x2004] = 'WAVE_FORMAT_COOK';
		$RIFFwFormatTagLookup[0x2005] = 'WAVE_FORMAT_DNET';
		...
		$RIFFwFormatTagLookup[0xFFFE] = 'WAVE_FORMAT_EXTENSIBLE';
		$RIFFwFormatTagLookup[0xFFFF] = 'WAVE_FORMAT_DEVELOPMENT';
	}

	return (isset($RIFFwFormatTagLookup[$wFormatTag]) ? $RIFFwFormatTagLookup[$wFormatTag] : 'unknown: 0x'.strpad(dechex($wFormatTag), 4, '0', STR_PAD_LEFT));
}

function RIFFfourccLookup($fourcc) {
	static $RIFFfourccLookup = array();
	if (empty($RIFFfourccLookup)) {
		$RIFFfourccLookup['swot'] = 'http://developer.apple.com/qa/snd/snd07.html';
